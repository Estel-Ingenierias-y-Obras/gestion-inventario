...
function Configuracion() {
  ...
  const loadUsers = useCallback(async () => {
    ...
    try {
      ...
      setToast({ type: 'error', message: error?.response?.data?.message || 'No se pudo cargar la whitelist.' });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async () => {
    ...
    if (deleteTarget.isPrimaryAdmin) {
      setDeleteTarget(null);
      setToast({ type: 'error', message: 'No se puede eliminar el administrador principal' });
      return;
    }

    setSaving(true);
    try {
      await deleteWhitelistUser(deleteTarget._id);
      setDeleteTarget(null);
      await loadUsers();
      setToast({ type: 'success', message: 'Usuario eliminado correctamente' });
    } catch (error) {
      setToast({ type: 'error', message: error?.response?.data?.message || 'Error al eliminar usuario' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <PageShell title="Configuración" subtitle="Gestión de usuarios autorizados para acceder a la aplicación" actions={<button className="button button--primary" type="button" onClick={() => setAddOpen(true)}>+ Añadir usuario</button>}>
      <section className="panel">
        <div className="panel__header panel__header--compact"><div><h2>Whitelist</h2><p>{users.length} usuarios autorizados</p></div></div>
        {loading ? <div className="panel__body"><LoadingState rows={5} /></div> : (
          <div className="table-scroll">
            <table className="data-table whitelist-table">
              <thead><tr><th>Nombre</th><th>Email</th><th>Fecha alta</th><th className="actions-column">Acciones</th></tr></thead>
              <tbody>{users.map((user) => (
                <tr key={user._id}>
                  <td><strong>{user.name}</strong>{user.isPrimaryAdmin ? <span className="table-secondary">Administrador principal</span> : null}</td>
                  <td>{user.email}</td>
                  <td>{new Date(user.createdAt).toLocaleDateString()}</td>
                  <td className="actions-column"><button className="icon-button icon-button--delete" type="button" disabled={user.isPrimaryAdmin} onClick={() => user.isPrimaryAdmin ? setToast({ type: 'error', message: 'No se puede eliminar el administrador principal' }) : setDeleteTarget(user)} aria-label={`Eliminar a ${user.name}`} title={user.isPrimaryAdmin ? 'El administrador principal no puede eliminarse' : 'Eliminar'}>🗑</button></td>
                </tr>
              ))}</tbody>
            </table>
          </div>
